...
class Robot2D:
    def __init__(self,
        ...
        """
        Args:
            num_joints (int, optional): _description_. Defaults to 3.
            init_states (_type_, optional): _description_. Defaults to torch.tensor([[2*np.pi/3,-np.pi/3,-np.pi/3]]).
            link_length (_type_, optional): _description_. Defaults to torch.tensor([[2,2,1]]).float().
            base_frame (str, optional): _description_. Defaults to 'default'.
    ...
    def forward_kinematics_eef(self,x):
        """
        Compute the forward kinematics for the end-effector of the robot.
        Args:
            x: Robot configuration (B, num_joints)
        Returns:
            f: Forward kinematics for the end-effector (B, 2)
        """
        self.B = x.size(0)
    ...
    def forward_kinematics_all_joints(self,x):
        """
        Compute the forward kinematics for all joints of the robot.
        Args:
            x: Robot configuration (B, num_joints)
        Returns:
    ...
    def forward_kinematics_any_point(self,x,a):
        """
        Compute the forward kinematics for any point along the robot's links.
        Args:
            x: Robot configuration (B, num_joints)
            a: Parameter to control the position along the links (0 <= a <= 1)
        Returns:
            F: Forward kinematics at the specified point (B, 2)
        """
    ...
    def surface_points_sampler(self, x, n=100):
        """
        Sample surface points along the robot's links.
        Args:
            x: Robot configuration (B, num_joints)
            n: Number of points to sample along each link
        Returns:
            kpts: Sampled points along the robot's links (B, 2, n)
        """
        self.B = x.size(0)
    ...
    def distance(self,x,p):
        """
        Compute the distance from a point p to the robot's surface points.
        Args:
            x: Robot configuration (B, num_joints)
            p: Point to compute distance to (B, 2)
        Returns:
            dist: Minimum distance from point p to the robot's surface points (B, n)
        """
        B = x.size(0)
